...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.hasPermission("Owner Of The Server" || "Staff" || "Manager Of Server" || "Head Of Moderators" || "Administrator")) {
            return message.reply("❌ You do not have permissions to kick members. Please contact a staff member")
                .then(m => m.delete(5000));
        }

        // No bot permissions
        if (!message.guild.me.hasPermission("DevBot")) {
        ...
        if (!toKick.kickable) {
            return message.reply("I can't kick that person due to their permsissions, I suppose.")
                .then(m => m.delete(5000));
        }
